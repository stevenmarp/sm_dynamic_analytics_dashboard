 ...
 * Chart.js v4 is responsive, so it follows the GridStack item as it resizes.
 */
export class SmAnalyticsChart extends Component {
    static template = "sm_dynamic_analytics_dashboard.Chart";
    static props = { block: Object, onDrill: Function, palette: { type: Array, optional: true } };
    ...
    renderChart() {
        ...
        const scales = cartesian ? {
            y: { beginAtZero: true, display: !isSpark, grid: { color: gridColor, display: !isSpark }, ticks: { color: tickColor } },
            x: { display: !isSpark, grid: { color: gridColor, display: !isSpark }, ticks: { color: tickColor } },
        } : {};
            ...
            options: {
                ...
                onClick: (ev, elements) => {
                    if (elements && elements.length) {
                        this.props.onDrill(block, elements[0].index);
                    }
                },
                plugins: {
                    legend: { display: multiColor, position: "right", labels: { color: tickColor } },
                    tooltip: { enabled: !isSpark },
                },
                scales,
            },
        });
...
export class SmDynamicAnalyticsDashboard extends Component {
    static template = "sm_dynamic_analytics_dashboard.Dashboard";
    static components = { SmAnalyticsChart };
    ...
    _dashboardId() {
        return this.state.dashboard?.id || this.props.action?.params?.dashboard_id || false;
    }

    _scheduleRefresh() {
        this._clearRefresh();
        const seconds = this.state.dashboard?.refresh_interval || 0;
        if (seconds > 0) {
    ...
    _onGridChange(items) {
        for (const item of items || []) {
            const id = Number(item.el?.getAttribute("gs-id"));
    ...
    smTargetPercent(block) {
        ...
        const ratio = (Number(block.data?.total || 0) / target) * 100;
    ...
    smMatrixMax(block) {
        const matrix = block.data?.matrix || [];
        let max = 0;
    ...
    smDrill(block, index) {
        ...
        const domains = block.data?.domains || [];
        this._openRecords(block, domains[index] || block.data?.base_domain);
    }
    ...
    smDrillTotal(block) {
        ...
        this._openRecords(block, block.data?.base_domain);
    }

    _openRecords(block, domain) {
        const model = block.data?.model;
        if (!model || !domain) {
            return;
        }
